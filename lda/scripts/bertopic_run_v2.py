...
def main():
    config = load_config()
    logger = setup_logger("bertopic_v2")
    ...
    log_memory_usage(memory_log, "start_bertopic_v2", logger)
    ...
    log_memory_usage(memory_log, "model_loaded_v2", logger)
    ...
    logger.info("Entrenando BERTopic v2 (más granular)...")
    ...
    model_dir = models_dir / "bertopic_v2"
    ...
    topic_info_path = outputs_dir / "bertopic_v2_topic_info.csv"
    ...
    keywords_path = outputs_dir / "bertopic_v2_topics_keywords.csv"
    ...
    papers_path = outputs_dir / "bertopic_v2_papers_topics.csv"
    ...
    for t in topics_keywords:
        ...
        graph_data.append({
            'id': f"BTv2_{topic_id:02d}",
            'topic_id': int(topic_id),
            'name': f"BERTopic_v2_{topic_id}",
            'keywords': t['keywords'],
            'num_papers': len(papers_list),
            'papers': papers_list
        })
    
    graph_path = outputs_dir / "bertopic_v2_topics_for_graph.json"
    with open(graph_path, 'w', encoding='utf-8') as f:
        json.dump(graph_data, f, ensure_ascii=False, indent=2)
    
    log_memory_usage(memory_log, "end_bertopic_v2", logger)
    logger.info("BERTopic v2 completado")
