...
def main():
    config = load_config()
    logger = setup_logger("bertopic_v3")
    ...
    log_memory_usage(memory_log, "start_bertopic_v3", logger)
    
    lemmatized_path = data_dir / "05_lemmatized_clean.parquet"
    df = pd.read_parquet(lemmatized_path)
    logger.info(f"Cargados {len(df)} documentos")
    
    docs = [" ".join(tokens) for tokens in df['tokens_clean']]
    
    logger.info("Cargando modelo de embeddings multilingüe...")
    embedding_model = SentenceTransformer("paraphrase-multilingual-MiniLM-L12-v2")
    
    log_memory_usage(memory_log, "model_loaded_v3", logger)
    ...
    logger.info("Entrenando BERTopic v3 (textos limpios)...")
    ...
    model_dir = models_dir / "bertopic_v3"
    ...
    topic_info_path = outputs_dir / "bertopic_v3_topic_info.csv"
    ...
    keywords_path = outputs_dir / "bertopic_v3_topics_keywords.csv"
    ...
    papers_path = outputs_dir / "bertopic_v3_papers_topics.csv"
    ...
    for t in topics_keywords:
        ...
        graph_data.append({
            'id': f"BTv3_{topic_id:02d}",
            'topic_id': int(topic_id),
            'name': f"BERTopic_v3_{topic_id}",
            'keywords': t['keywords'],
            'num_papers': len(papers_list),
            'papers': papers_list
        })
    
    graph_path = outputs_dir / "bertopic_v3_topics_for_graph.json"
    with open(graph_path, 'w', encoding='utf-8') as f:
        json.dump(graph_data, f, ensure_ascii=False, indent=2)
    
    log_memory_usage(memory_log, "end_bertopic_v3", logger)
    logger.info("BERTopic v3 completado")
